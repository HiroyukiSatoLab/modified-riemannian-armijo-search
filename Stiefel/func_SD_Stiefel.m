%% Riemannian steepest descent with Armijo backtracking on the Stiefel manifold

function [k, bt, time] = func_SD_Stiefel(n, sizer, eps, beta, tau, flag)
% Inputs:
%   n    : problem size
...
while norm(Rgrad, 'fro') >= eps
    ...
    r = 0;
    while 1
        Xp = qr_unique(X + t*p);     % Retraction based on QR decomposition
        r = r + 1;
        AXpN = A*Xp*N;
        fXp = .5 * Xp(:)' * AXpN(:); % f(R_x(tp))
